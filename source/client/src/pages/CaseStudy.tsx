...
type Study = {
  slug: string;
  index: string;
  profile: "Markets" | "AI Engineering";
  eyebrow: string;
  title: string;
  status: string;
  summary: string;
...
const studies: Record<string, Study> = {
  rxcode: {
    ...
    summary: "A native macOS command center for teams that need to run coding agents with visible sessions, reviewable changes, and explicit permission decisions.",
    ...
    evidence: "The product website and public repository are available. The project is framed as an active, open-source product; this page deliberately does not invent adoption, revenue, or performance metrics beyond public materials.",
  ...
  "sport-ai": {
    ...
    evidence: "The supplied experience evidence reports 63 validated events and 10 passing regression tests. The scope is described as an evidence-aware content system, not a general-purpose autonomous commentator.",
  ...
  polyalpha: {
    ...
    evidence: "The project presentation reports 400M+ historical trades, a 62.9% reported win rate, and a 2.53 reported Sharpe ratio. Those figures are retained as database or research outputs only; they are not claims of live trading performance.",
  ...
  "rmbi-alpha": {
    slug: "rmbi-alpha", index: "05", profile: "Markets", eyebrow: "Factor research / walk-forward evaluation", title: "RMBI Alpha Challenge", status: "Research build",
    summary: "A composite-factor research build centred on out-of-sample evaluation, interpretability, and regime awareness rather than a single headline return figure.",
    ...
    implementation: "The work engineered six composite factors and evaluated a HistGradientBoostingRegressor using a walk-forward process. The build focused on systematic preparation, testing across periods, and clearly separating baseline comparison from model output.",
    evidence: "The portfolio reports a 1.20 out-of-sample Sharpe, a 0.66 SPY baseline, and 19 years of out-of-sample data. These are research results from the stated study, not an investable strategy or future-performance claim.",
    note: "Research disclosure: results are shown to communicate method and evaluation discipline, not to market an investment strategy.",
    links: [],
  },
};
...
export default function CaseStudy() {
  ...
  const artifact = {
    rxcode: ["SESSION", "DIFF", "APPROVE"],
    "sport-ai": ["TRAJECTORY", "MATCH", "FALLBACK"],
    polyalpha: ["DATA", "VALIDATE", "TESTNET"],
    stubase: ["REPORT", "SCREEN", "EVIDENCE"],
    "rmbi-alpha": ["FACTOR", "WALK-FORWARD", "OOS"],
  }[study.slug] ?? ["INPUT", "REVIEW", "OUTPUT"];
  const rxcodeVisual = typeof window !== "undefined" && window.location.hostname.endsWith("github.io")
    ? "/assets/rxcode-agent-control-demo.png"
    : "/manus-storage/rxcode-agent-control-demo_2b76d471.png";
  ...
  return (
    <div className={`app-shell case-page ${study.profile === "AI Engineering" ? "case-ai" : "case-markets"}`}>
      ...
      <main className="case-main">
        <div className="case-route-rail" aria-hidden="true"><span /><i /><b /></div>
        <a className="case-back" href={backHref}><ArrowLeft size={16} /> {backLabel}</a>
        <section className="case-hero"><div><p className="case-kicker">CASE {study.index} / {study.eyebrow}</p><p className="case-coordinate">ROUTE NODE {study.index} / EVIDENCE RECORD / {study.profile === "Markets" ? "MARKETS" : "AI OPS"}</p><h1>{study.title}</h1><p className="case-summary">{study.summary}</p></div><aside className="case-status"><span>PROFILE</span><strong>{study.profile}</strong><span>STATUS</span><strong>{study.status}</strong><span>MATURITY</span><strong>{study.status.includes("testnet") ? "Research boundary" : "Public evidence linked"}</strong></aside></section>
        <section className="case-instrument" aria-label={`${study.title} project instrument`}>
          <div className="case-instrument-head"><span>PROJECT ARTIFACT</span><strong>{study.profile === "Markets" ? "Research route" : "Agent control route"}</strong><em>COORD {study.index}.02</em></div>
          {study.slug === "rxcode" ? <img src={rxcodeVisual} alt="Conceptual RxCode agent-control workspace visual" /> : null}
          <div className="case-artifact-chain">{artifact.map((item, index) => <span key={item}><b>{String(index + 1).padStart(2, "0")}</b>{item}</span>)}</div>
          <p>{study.profile === "Markets" ? "A bounded research trail: source data, validation, and a clearly stated route to use." : "A bounded operating loop: system input, a visible review gate, and a traceable output state."}</p>
        </section>
        <section className="case-grid" aria-label={`${study.title} project record`}><article className="case-block"><span><Lightbulb size={16} /> Problem</span><p>{study.problem}</p></article><article className="case-block"><span><Wrench size={16} /> Implementation</span><p>{study.implementation}</p></article><article className="case-block"><span><ShieldCheck size={16} /> Evidence</span><p>{study.evidence}</p></article><article className="case-block case-links"><span><ArrowUpRight size={16} /> Links</span>{study.links.length ? <div>{study.links.map((link) => <a key={link.label} href={link.href} target={link.href.startsWith("http") ? "_blank" : undefined} rel={link.href.startsWith("http") ? "noreferrer" : undefined}>{iconFor(link.kind)} {link.label} <ArrowUpRight size={14} /></a>)}</div> : <p>Research record is summarised on this page; no public project endpoint is linked.</p>}</article></section>
        {study.note ? <p className="case-disclosure"><FileText size={15} /> {study.note}</p> : null}
      </main>
